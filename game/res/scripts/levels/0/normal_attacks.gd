...
func bullet_hell_semicircle(userdata: Dictionary) -> void:
	...
	while continue_attacking:
		...
		BulletPatternManager.bullet_hell_semicircle(
			userdata.BULLET_TYPE, total_number_of_bullets, starting_angle, [params_dict]
		)
...
func three_lane_fireball_secondary_attack(userdata: Dictionary):
	...
	var x_max_value: float = (Globals.SCREEN_SIZE.x / 2) * 0.9
	var x_min_value: float = (Globals.SCREEN_SIZE.x / 2) * 0.6
	var y_max_value: float = (Globals.SCREEN_SIZE.y / 2) * 0.9
	var y_min_value: float = (Globals.SCREEN_SIZE.y / 2) * 0.6

	var next_position_x: int = [
		rng.randi_range(x_max_value, x_min_value),
		rng.randi_range(-x_max_value, -x_min_value)
	][randi()%2]
	var next_position_y: int = [
		rng.randi_range(y_max_value, y_min_value),
		rng.randi_range(-y_max_value, -y_min_value)
	][randi()%2]
	...
	while continue_attacking:
		...
		BulletPatternManager.bullet_hell_circle(
			userdata.BULLET_TYPE, 10, [params_dict]
		)
...
func constant_fire_circle(userdata: Dictionary) -> void:
	...
	while continue_attacking:
		...
		params_dict.base_speed = userdata.BASE_BULLET_SPEED + (
			50 - clamp(500 - current_damage_taken, 0, 500) / 10
		)
		main_attack_timer.set_wait_time(wait_time_between_bullets)

		if not is_defeated:
			for c in range(userdata.n_of_bullets):
				params_dict["global_position"] = arena_center + radius.rotated(
					step * c + offset * (current_iteration % 15)
				)
				bullet_angle_to_screen_center = (
					arena_center - params_dict["global_position"]
				).angle()
				params_dict["velocity"] = Vector2(
					cos(bullet_angle_to_screen_center),
					sin(bullet_angle_to_screen_center)
				) * userdata.BASE_BULLET_SPEED
				new_bullet = SimpleBulletManager.get_avaliable_bullet(
					userdata.BULLET_TYPE, params_dict
				)
