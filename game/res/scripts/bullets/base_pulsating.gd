...
const BASE_PROYECTILES_PER_EXPLOSION: int = 10
...
var bullet_types_list: Array = []
var proyectiles_per_explosion: int = -1
...
func explode() -> void:
	...
	while (
		(max_bursts == -1 and keep_shooting) or
		(max_bursts != -1 and current_burst < max_bursts)
	):
		if random_proyectile_types_enabled:
			current_shooting_bullet_type = bullet_types_list[randi()%2]
		...
		BulletPatternManager.bullet_hell_circle(
			current_shooting_bullet_type,
			proyectiles_per_explosion if proyectiles_per_explosion != -1 else BASE_PROYECTILES_PER_EXPLOSION,
			[params_dict]
		)
		current_burst += 1
		rotation_direction *= -1
		...
		if max_bursts == -1:
			burst_delay = base_burst_delay + TIMER_INCREMENT * (clamp(
				LIFE_THRESHOLD - EnemyManager.get_enemy_current_damage_taken(),
				0, LIFE_THRESHOLD
			) / LIFE_THRESHOLD)
