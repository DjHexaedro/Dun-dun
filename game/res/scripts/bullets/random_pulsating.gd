...
var screen_center_position: Vector2 = Vector2.ZERO
...
func set_properties(params_dict: Dictionary) -> void:
	...
	keep_shooting = true
	screen_center_position = ArenaManager.get_current_location().get_enemy_positions_list().global.SCREEN_CENTER
...
func explode() -> void:
	...
	var angle_to_center: float = (screen_center_position - global_position).angle()
