	...
	if can_move:
		...
		if can_move_y:
			if only_vertical_movement:
				...
				if position.y <= Globals.Positions.TOP_Y:
					position.y = Globals.Positions.TOP_Y
					movement_direction *= -1
				elif position.y >= Globals.Positions.BOTTOM_Y:
					position.y = Globals.Positions.BOTTOM_Y
					movement_direction *= -1
			else:
				position.y += speed.y * delta * (1 if position.x == Globals.Positions.RIGHT_X else -1)
				if position.y <= Globals.Positions.TOP_Y:
					position.y = Globals.Positions.TOP_Y
					can_move_y = false
					can_move_x = true
			...
			position.x += speed.x * delta * (1 if position.y == Globals.Positions.TOP_Y else -1)
			if position.x <= Globals.Positions.LEFT_X:
				position.x = Globals.Positions.LEFT_X
				can_move_y = true 
				can_move_x = false 
			elif position.x >= Globals.Positions.RIGHT_X:
				position.x = Globals.Positions.RIGHT_X
				can_move_y = true 
				can_move_x = false 
...
func explode() -> void:
	...
	var base_angle_to_screen_center: float = (Globals.Positions.SCREEN_CENTER - position).angle()
