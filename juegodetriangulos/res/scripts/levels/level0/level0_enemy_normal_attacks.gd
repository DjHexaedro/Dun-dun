# Attacks used by the normal version of the boss
extends "res://juegodetriangulos/res/scripts/levels/level0/base_level0_enemy.gd"
...
# they spawn and the enemy dies the must be manually removed
...
func fire_rain(userdata: Dictionary) -> void:
	...
	var parent_attack_id: int = chosen_attack_id
	while continue_attacking:
		...
		SimpleBulletManager.shoot_bullet(userdata.BULLET_TYPE, params_dict, false)
		...
		if parent_attack_id != chosen_attack_id:
			break
...
func shoot_soft_homing_bullet(userdata: Dictionary) -> void:
	var new_x_position: int = position.x
	var new_y_position: int = position.y
	var params_dict: Dictionary = {
		"global_position": position,
		...
		"damage": 1,
		"where_goes": "goes_up" if new_y_position == Globals.Positions.BOTTOM_Y else "goes_down",
	...
	while continue_attacking:
		if get_random_x_position:
			new_x_position = rng.randi_range(PlayerManager.get_player_position().x - userdata.SPAWN_POSITION_MARGIN_X, PlayerManager.get_player_position().x + userdata.SPAWN_POSITION_MARGIN_X)
		if current_attack_stage > 15:
			new_y_position = [Globals.Positions.BOTTOM_Y, Globals.Positions.TOP_Y][randi()%2]
			params_dict.where_goes = "goes_up" if new_y_position == Globals.Positions.BOTTOM_Y else "goes_down"
...
func three_lane_fireball_secondary_attack(userdata: Dictionary):
	...
	var next_position_x: int = [rng.randi_range(100, Globals.SCREEN_SIZE.x * 0.2), rng.randi_range(Globals.SCREEN_SIZE.x * 0.8, Globals.SCREEN_SIZE.x)][randi()%2]
	var next_position_y: int = [rng.randi_range(100, Globals.SCREEN_SIZE.y * 0.3), rng.randi_range(Globals.SCREEN_SIZE.y * 0.7, Globals.SCREEN_SIZE.y)][randi()%2]
	while continue_attacking:
		...
		next_position_x = [rng.randi_range(100, Globals.SCREEN_SIZE.x * 0.2), rng.randi_range(Globals.SCREEN_SIZE.x * 0.8, Globals.SCREEN_SIZE.x)][randi()%2]
		next_position_y = [rng.randi_range(100, Globals.SCREEN_SIZE.y * 0.3), rng.randi_range(Globals.SCREEN_SIZE.y * 0.7, Globals.SCREEN_SIZE.y)][randi()%2]
...
func fireball_circle(userdata: Dictionary) -> void:
	...
	var params_dict: Dictionary = {
		"global_position": Globals.Positions.SCREEN_CENTER,
...
func setup_attack_constants() -> void:
	...
	attack_params["three_lane_fireball"] = {
		...
		LOWER_MINION_PARAMS = {
			...
			"max_attack_speed": 2,
			"min_attack_speed": 2,
			"position": Vector2(Globals.SCREEN_SIZE.x * 0.625, Globals.SCREEN_SIZE.y * 0.9),
		},
		UPPER_MINION_PARAMS = {
			"bullets_per_shot": 4,
			"bullet_size": Globals.BulletSizes.QUADRUPLE,
			...
			"max_attack_speed": 2,
			"min_attack_speed": 2,
			"position": Vector2(Globals.SCREEN_SIZE.x * 0.375, Globals.SCREEN_SIZE.y * 0.1),
		}
	}
